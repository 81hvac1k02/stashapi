...
class ScrapeParser:
    ...
    def gallery_from_scrape(self, gallery):
        ...
        if gallery.get("performers"):
            gallery_update["performer_ids"] = [
                self.performer_from_scrape(p)["id"] for p in gallery["performers"]
            ]
        if gallery.get("studio"):
            gallery_update["studio_id"] = self.studio_from_scrape(
                gallery["studio"]
            ).get("id", None)
    ...
    def performer_from_scrape(self, performer):
        ...
        if performer.get("weight"):
            try:
                ...
                log.warning(
                    f'Could not parse performer weight "{performer["weight"]}" it Int for {performer["name"]}'
                )
        ...
        if performer.get("gender"):
            try:
                ...
                log.warning(
                    f'Cannot map performer Gender "{performer["gender"]}" for {performer["name"]}'
                )
    ...
    def scene_from_scrape(self, scene):
        ...
        if scene.get("studio"):
            scene_update["studio_id"] = self.studio_from_scrape(scene["studio"]).get(
                "id"
            )
        ...
        if scene.get("movies"):
            scene_update["movies"] = [
                self.scene_movie_input_from_scrape(m) for m in scene["movies"] if m
            ]
