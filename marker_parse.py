...
class Marker:
    ...
    def __eq__(self, other) -> bool:
        return (
            ...
            and self.primary_tag["id"] == other.primary_tag["id"]
        )
    ...
    def within_distance(self, other, seconds_distance: int = 15):
        if not isinstance(other, Marker):
            raise ValueError(
                f"Marker.within_distance() must compare to <Marker> type not {type(other)}"
            )
    ...
    def from_scrape(cls, scraped: dict, scene_id, stash: StashInterface):
        ...
        if isinstance(seconds, (int, float)):
            ...
            try:
                ...
                raise ValueError(
                    f"Could not cast <Marker>.seconds string to float: '{seconds}' =!=> float()"
                )
        else:
            raise ValueError(
                f"<Marker>.seconds has unexpected type {seconds}({type(seconds)})"
            )
        ...
        if not primary_tag:
            raise Exception(
                f'Could not find/create tag for <primary_tag>:"{scraped["primary_tag"]["name"]}"'
            )
...
def import_scene_markers(
    stash: StashInterface,
    scraped_markers,
    stash_scene_id,
    closest_allowed_common_marker: int = 15,
):
    """
    ...
    mapped_markers = [
        Marker.from_scrape(m, stash_scene_id, stash) for m in scraped_markers
    ]
    stash_markers = [
        Marker.from_gql(m)
        for m in stash.get_scene_markers(stash_scene_id, fragment=SCENE_MARKER_FRAGMENT)
    ]
    ...
    for scraped in mapped_markers:
        ...
        within_limit = [
            existing.seconds
            for existing in stash_markers
            if scraped.within_distance(existing, closest_allowed_common_marker)
        ]
        if within_limit:
            log.debug(
                f'Skipped Tag: {scraped.primary_tag["name"]} {scraped.seconds} +/- {closest_allowed_common_marker}(s) of {within_limit}'
            )
            continue

        stash_marker = stash.create_scene_marker(
            scraped.gql_input(), SCENE_MARKER_FRAGMENT
        )
        if not stash_marker:
    ...
    log.info(
        f"created ({len(new_marker_list)}) new marker(s) for SceneID ({stash_scene_id})"
    )
    new_marker_log = [
        f'{m["primary_tag"]["name"]}@{m["seconds"]}' for m in new_marker_list if m
    ]
